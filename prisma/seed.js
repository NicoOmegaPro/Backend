...
async function main() {
  ...
  const subtareas = await createRandomSubtareas(prisma, randomTasks, 2);
  console.log(`${subtareas.length} subtareas aleatorias creadas`);

  console.log('Ejecutando ComentariosFactory...');
  const comentarios = await createRandomComentarios(prisma, randomTasks, allUsers, 1);
  ...
  const notificaciones = await createRandomNotificaciones(prisma, allUsers, 25);
  console.log(`${notificaciones.length} notificaciones aleatorias creadas`);

  console.log('Ejecutando HistorialFactory...');
  const historial = await createRandomHistorial(prisma, allUsers, 25);
